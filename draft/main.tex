...
Repeating this residual analysis on multiple samples from the posterior showed that only 4.2\% of 
...
generate a posterior distribution of p-values (TODO cites).

If on the other hand $\mathcal{D}$ is the result of maximum likelihood estimation,
then first we need to generate samples from the posterior before residuals can be computed.
\footnote{Could mention somewhere case of a summary tree from the posterior distribution \citep{heledLookingTreesForest2013}}
...
Maybe use improper $\mathrm{InvGamma}(0,\infty)$ prior on $\alpha$ so that posterior is:
\begin{equation}
