...
then this is readily available for validation and we therefore focus here on this case
...
TODO. Maybe using simulation with relaxed clock model and inference with strict clock model.
...
In this case the distribution of $l_i$ is discrete, but let us for now consider that the distribution
...
including a continuous version of the strict clock model based on a Gamma process:
...
Let us consider the general case where
...
We can also define the normal residuals $n_i$, analogous to the residuals commonly used in 
...
as Norm(0,1). The uniform and normal residuals above can be computed directly
when the clock model is continuous (for example Equation \ref{eq:csc}) but when
the clock model is discrete (for example Equation \ref{eq:sc}) we need to make the following
...
distribution with the theoretical Unif(0,1). The normal residuals $n_i$ can be plotted
...
Currently using Shapiro-Wilk test for normality combined with Kolmogorov-Smirnov Test.
The former is more powerful to test Normality, but only tests if Normal, not if Normal(0,1),
hence combination with latter.

Note that in \citep{lauNewModelDiagnostics2014} they use an Anderson-Darling test and cite
Lewis PAW. 1961 Distribution of the AndersonDarling statistic. Ann. Math. Stat. 32, 1118 –1124.

Use of residuals in Hidden Markov Models \citep{zucchini2009hidden,buckbyModelCheckingHidden2020}.
