...
o=capture.output(rtd<-suppressWarnings(dater(tre,sts,s=l)))
print(sprintf('Rate=%f and root date=%f',rtd$mean.rate*l,rtd$timeOfMRCA))
```
...
o=capture.output(result <- lsd2(inputTree='/tmp/tree.nwk', inputDate='/tmp/dates.csv',outFile = '/tmp/result', seqLen=l))
print(sprintf('Rate=%f and root date=%f',result$rate*l,result$tMRCA))
```