...
allres <- foreach (rep = 1:100,.packages = c('ape','BactDating','ValidateDating')) %dopar% {
  ...
  v1=validate(r1,resampling = 0,showPlot = F);p1=length(which(v1<0.05))/length(v1)
  r2=runDating(phy,dates,algo='treedater')
  v2=validate(r2,resampling = 2,showPlot = F);p2=length(which(v2<0.05))/length(v2)
  r3=runDating(phy,dates,algo='node.dating')
  v3=validate(r3,resampling = 2,showPlot = F);p3=length(which(v3<0.05))/length(v3)
  r4=runDating(phy,dates,algo='TreeTime')
  v4=validate(r4,resampling = 2,showPlot = F);p4=length(which(v4<0.05))/length(v4)
  r1=runDating(phy,dates,rate=10)
  v1=validate(r1,resampling = 0,showPlot = F);p1f=length(which(v1<0.05))/length(v1)
  r2=runDating(phy,dates,algo='treedater',rate=10)
  v2=validate(r2,resampling = 2,showPlot = F);p2f=length(which(v2<0.05))/length(v2)
  r3=runDating(phy,dates,algo='node.dating',rate=10)
  v3=validate(r3,resampling = 2,showPlot = F);p3f=length(which(v3<0.05))/length(v3)
  r4=runDating(phy,dates,algo='TreeTime',rate=10)
  v4=validate(r4,resampling = 2,showPlot = F);p4f=length(which(v4<0.05))/length(v4)
