...
allres <- foreach (rep = 1:100,.packages = c('ape','BactDating','ValidateDating')) %dopar% {
  ...
  v1=validate(r1);p1=length(which(v1<0.05))/length(v1)
  r2=runDating(phy,dates,algo='treedater')
  v2=validate(r2);p2=length(which(v2<0.05))/length(v2)
  r3=runDating(phy,dates,algo='node.dating')
  v3=validate(r3);p3=length(which(v3<0.05))/length(v3)
  r4=runDating(phy,dates,algo='TreeTime')
  v4=validate(r4);p4=length(which(v4<0.05))/length(v4)
